...
def cross_validation(features, labels, n_folds):
    ...
    # let x = np.array, l = len(x), i = an intager greater or equal to l
    # np.array[:i] => will return the full VIEW of the array 
    # np.array[i:] => will return an empty array
    ...
    res = []
    start = 0
    num_of_data_per_fold = len(features) // n_folds # len(features) = len(label)
    while start < len(features):
        test_features = features[start:start+num_of_data_per_fold]
        test_labels = labels[start:start+num_of_data_per_fold]
        train_features = np.concatenate((features[0:start], features[start+num_of_data_per_fold:]), axis=0)
        train_labels = np.concatenate((labels[0:start], labels[start+num_of_data_per_fold:]))
        res.append((train_features, train_labels, test_features, test_labels))
        start += num_of_data_per_fold
    return res 
