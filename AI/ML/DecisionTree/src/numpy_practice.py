...
def divide_matrix_along_rows(x, y):
    ...
    """
    #Practice Notes:
    #   y.reshape(len(x), 1) will convert one dimensional array into a column vector(or a matrix with the following dimeansions (3,1))
    #   when x/y is executed numpy will broadcast y's each row vector to match the length of x's column
