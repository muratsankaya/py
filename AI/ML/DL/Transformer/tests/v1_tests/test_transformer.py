...
def test_transformer_dropout_parameter(dropout_p):
    """
    Test that the Transformer model can properly handle various dropout values,
    and that setting dropout changes the forward pass outputs in a stochastic manner.

    Parameters:
    - dropout_p (float): The dropout probability to apply.
    """
    batch_size = 4
    seq_len = 8
    d_model = 64
    ...
    outputs = []
    for _ in range(3):
        out = transformer(encoder_x, decoder_x)
        outputs.append(out.detach().clone())

    # If dropout_p is 0.0, subsequent outputs should be identical (deterministic)
    if dropout_p == 0.0:
        assert torch.allclose(outputs[0], outputs[1]) and torch.allclose(
            outputs[1], outputs[2]
        ), "Outputs differ across runs even though dropout is 0.0, which should not happen."
    else:
        # With dropout > 0, there's a high likelihood that at least one output differs
        # This isn't guaranteed at a small number of runs but it's statistically very likely.
        # We'll just check that not all outputs are identical.
        all_identical = True
        ...
        assert not all_identical, (
            ...
            "which is statistically unlikely and suggests dropout may not be applied."
        )
