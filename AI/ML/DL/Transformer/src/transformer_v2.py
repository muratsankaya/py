"""
This is the 2nd version of our Transformer Implementation.
Its specially fine tuned to be used with the helinski_nlp_opus tokenizer 
"""
...
def attention(
    ...
    x = x.masked_fill(pad_mask == 0, float("-inf"))
...
class Decoder(torch.nn.Module):
    ...
    def forward(
        ...
        # Cross-Attention
        # Here encoder_pad_mask must be used
...
# TODO: try writing positional encodings with pytorch.
# numpy could be removed from this project.
# removing it would save space.
...
class Transformer(torch.nn.Module):
    ...
    def forward(
        ...
        inputs:
            ...
            encoder_pad_mask: a torch tensor of size: (batch_size, seq_len)
            decoder_pad_mask: a torch tensor of size: (batch_size, seq_len)
        ...
        encoder_pad_mask = (
            encoder_pad_mask.view(self.batch_size, 1, 1, self.seq_len)
            .expand(-1, self.number_of_heads, self.seq_len, -1)
            .reshape(self.batch_size * self.number_of_heads, self.seq_len, self.seq_len)
        )

        decoder_pad_mask = (
            decoder_pad_mask.view(self.batch_size, 1, 1, self.seq_len)
            .expand(-1, self.number_of_heads, self.seq_len, -1)
            .reshape(self.batch_size * self.number_of_heads, self.seq_len, self.seq_len)
        )

        for encoder in self.encoders:
            encoder_x = encoder(encoder_x, encoder_pad_mask, dropout_p=self.dropout_p)
